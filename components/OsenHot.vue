<template>
    <div v-if="rows" class="HotBox vspace20">
        <h5><img :src="staticDomain +'/web2/images/common/h5__hot.gif'" alt=" 핫!!!"/></h5>
        <ul>
            <li v-for="row in rows" :key="row">
                <a :href="'/photo/slide_view/'+ row.gisano">
                    <img :src="row.thumbUrl1" />
                </a>
                <p class="sbj"><a :href="'/photo/slide_view/'+ row.gisano">{{ row.title }}</a></p>
            </li>
        </ul>
    </div>
</template>
